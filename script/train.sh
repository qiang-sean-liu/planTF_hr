...
SAVE_DIR="/mnt/nas20/qiang.liu/plantf_training_save_dir/plantf_stateAttOn_natOn_posEncOff"

CUDA_VISIBLE_DEVICES=0,1,2,3 python run_training.py \
  group=$SAVE_DIR \
  py_func=train +training=train_planTF \
  worker=single_machine_thread_pool worker.max_workers=32 \
  scenario_builder=nuplan cache.cache_path=/mnt/nas25/nuplan_cached/cache_plantf_1M cache.use_cache_without_dataset=true \
